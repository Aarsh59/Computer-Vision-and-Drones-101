...
def hybrid(s1,s2):
   ...
   #resizing to 256 by 256
    ...
    cv2.imshow('Input Dog',img5)
    cv2.imshow('Input Cat',img6)
# convert to gray scale
    img7 = cv2.cvtColor(img5,cv2.COLOR_BGR2GRAY)
    img8 = cv2.cvtColor(img6,cv2.COLOR_BGR2GRAY)
    cv2.imshow('Graysacle Dog',img7)
    cv2.imshow(' Grayscale Cat',img8)
# finding fourier of two images
  ...
  # fourier transform of both images
    cv2.imshow('Fourier Dog',magnitude)
    cv2.imshow('Fourier Cat',magnitude1)
    # creating a low pass and high pass filter
    rows , col = img7.shape
    ...
    cv2.imshow('Fourier Dog after filter',f1_after)
    cv2.imshow('fourier Cat after filter',f2_after)
    #Finding inverse fourier transfom
    ...
    cv2.imshow('modified Dog',img1_back)
    cv2.imshow('modified Cat',img2_back)
    #Final Image
...
hybrid(r"C:\Users\HP\Downloads\download (3).jpeg", r"C:\Users\HP\Downloads\download (2).jpeg")     
